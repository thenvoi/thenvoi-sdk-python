...
from .remote_agent import GatewayClientError, format_gateway_error_for_user
...
class OrchestratorAgentExecutor(AgentExecutor):
    ...
    async def execute(
        ...
        try:
            ...
            logger.warning(
                "Gateway call failed during orchestration: code=%s peer=%s retryable=%s",
                error.code,
                error.peer_id,
                error.retryable,
            )
            await updater.update_status(
                TaskState.input_required,
                new_agent_text_message(
                    format_gateway_error_for_user(error),
                    task.context_id,
                    task.id,
                ),
                final=True,
            ...
            logger.error("Error executing orchestrator agent: %s", error)
            raise ServerError(error=InternalError()) from error
