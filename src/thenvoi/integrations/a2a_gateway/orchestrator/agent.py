...
def make_call_peer_agent_tool(gateway_client: GatewayClient) -> Any:
    """Create a call_peer_agent tool bound to a specific gateway client."""

    @tool("call_peer_agent")
    async def call_peer_agent(
        ...
        return await gateway_client.call_peer(
            peer_id,
            message,
            context_id=context_id,
        )

    return call_peer_agent
...
Always be helpful and provide clear responses to the user."""
    ...
    def __init__(
        ...
        # Each orchestrator owns its own gateway client and bound tool instance.
        ...
        self.tools = [make_call_peer_agent_tool(self.gateway_client)]
