...
class TestContactEventConfigValidation:
    ...
    def test_callback_rejects_sync_on_event(self):
        """Verify CALLBACK strategy rejects non-async callbacks."""

        def handler(event, tools):
            return None

        with pytest.raises(
            ValueError, match="CALLBACK strategy requires async on_event callback"
        ):
            ContactEventConfig(
                strategy=ContactEventStrategy.CALLBACK,
                on_event=handler,
