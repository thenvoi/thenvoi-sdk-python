...
class _SinkSpy:
    """Test double for the ContactEventSink interface."""

    def __init__(self, *, hub_enabled: bool) -> None:
    ...
    async def initialize_hub_room(self, room_id: str, system_prompt: str) -> None:
        self.initialized.append((room_id, system_prompt))

    async def inject_hub_event(self, room_id: str, event: object) -> None:
        self.injected.append((room_id, event))


class TestExplicitSink:
    """Tests for explicit sink injection contract."""

    def test_rejects_mixed_sink_and_callback_arguments(self, mock_link) -> None:
        """Handler should require either sink OR callbacks, not both."""
        config = ContactEventConfig(strategy=ContactEventStrategy.DISABLED)

        with pytest.raises(ValueError, match="either sink or callback arguments"):
            ContactEventHandler(
                config,
                mock_link,
                on_broadcast=lambda _msg: None,
                sink=_SinkSpy(hub_enabled=False),
            )

    async def test_broadcast_uses_sink_interface(
        self,
        mock_link,
        sample_contact_added_event,
    ) -> None:
        """Broadcasting should route through ContactEventSink.broadcast()."""
        config = ContactEventConfig(
            strategy=ContactEventStrategy.DISABLED,
            broadcast_changes=True,
        )
        sink = _SinkSpy(hub_enabled=False)
        handler = ContactEventHandler(config, mock_link, sink=sink)

        await handler.handle(sample_contact_added_event)

        assert sink.broadcast_messages == ["@bob (Bob) is now a contact"]
...
class TestCallbackStrategy:
    ...
    async def test_sync_callback_slipthrough_fails_gracefully(
        self, mock_link, sample_request_received_event
    ):
        """Non-awaitable callbacks should fail closed if misconfigured post-init."""

        async def async_handler(event, tools):
            return None

        def sync_handler(event, tools):
            return None
        ...
        # Simulate post-init mutation from external config wiring.
        config.on_event = sync_handler
        handler = ContactEventHandler(config, mock_link)

        await handler.handle(sample_request_received_event)

        assert handler.is_event_processed(sample_request_received_event) is False
...
class TestDeduplication:
    ...
    async def test_dedup_cache_bounded(self, mock_link, mock_callback):
        ...
        assert handler.diagnostics().dedup_cache_size <= MAX_DEDUP_CACHE_SIZE
...
class TestDedupKeyGeneration:
    ...
    def test_get_dedup_key_request_received(self, mock_link):
        ...
        key = handler.dedup_key_for_event(event)
    ...
    def test_get_dedup_key_contact_added(self, mock_link, sample_contact_added_event):
        ...
        key = handler.dedup_key_for_event(sample_contact_added_event)
    ...
    def test_get_dedup_key_contact_removed(
        ...
        key = handler.dedup_key_for_event(sample_contact_removed_event)
...
class TestHubRoomStrategy:
    ...
    async def test_hub_room_error_returns_false(
        ...
        assert not handler.is_event_processed(sample_request_received_event)

    async def test_hub_room_records_nonfatal_when_task_event_post_fails(
        self, mock_hub_link, mock_hub_event_callback, sample_request_received_event
    ) -> None:
        """Task-event persistence failures should be recorded as nonfatal errors."""
        mock_hub_link.rest.agent_api_events.create_agent_chat_event = AsyncMock(
            side_effect=RuntimeError("task event failed")
        )
        config = ContactEventConfig(strategy=ContactEventStrategy.HUB_ROOM)
        handler = ContactEventHandler(
            config, mock_hub_link, on_hub_event=mock_hub_event_callback
        ...
        assert handler.is_event_processed(sample_request_received_event)
        assert handler.nonfatal_errors
        assert handler.nonfatal_errors[0]["operation"] == "post_task_event"
        assert handler.nonfatal_errors[0]["room_id"] == "hub-room-123"
        assert handler.nonfatal_errors[0]["event_type"] == "contact_request_received"
    ...
    async def test_hub_room_injects_system_prompt_on_first_event(
        ...
        from thenvoi.runtime.contacts.contact_handler import HUB_ROOM_SYSTEM_PROMPT
    ...
    async def test_hub_room_uses_explicit_sink_contract(
        self,
        mock_hub_link,
        sample_request_received_event,
    ) -> None:
        """HUB_ROOM strategy should work with a typed sink implementation."""
        from thenvoi.runtime.contacts.contact_handler import HUB_ROOM_SYSTEM_PROMPT

        config = ContactEventConfig(strategy=ContactEventStrategy.HUB_ROOM)
        sink = _SinkSpy(hub_enabled=True)
        handler = ContactEventHandler(config, mock_hub_link, sink=sink)

        await handler.initialize_hub_room()
        handler.mark_hub_room_ready()
        await handler.handle(sample_request_received_event)

        assert sink.initialized == [("hub-room-123", HUB_ROOM_SYSTEM_PROMPT)]
        assert len(sink.injected) == 1
        assert sink.injected[0][0] == "hub-room-123"
