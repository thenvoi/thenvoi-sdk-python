"""E2E tests for the Claude SDK adapter.

Verifies that the Claude SDK adapter can:
- Start, process a message, and stop against a real platform
- Execute platform tools via MCP (send_message)

Note: Claude SDK requires Node.js 20+ and the @anthropic-ai/claude-code CLI.

Run with:
    E2E_TESTS_ENABLED=true uv run pytest tests/e2e/adapters/test_claude_sdk.py -v -s --no-cov
"""
...
@requires_e2e
class TestClaudeSDKE2E:
    """E2E tests specific to the Claude SDK adapter."""
    ...
    async def test_smoke_responds_to_message(
        ...
        logger.info(
            "Claude SDK smoke test passed: received %d response(s)", len(received)
        )
    ...
    async def test_tool_execution_send_message(
        ...
        """Verify the agent uses MCP tools to send a message."""
        chat_id, user_id, user_name = e2e_chat_room_with_user
        adapter = claude_sdk_adapter_factory(e2e_config)
        ...
        assert len(received) > 0, "Agent should have sent a message via MCP tool"
        assert_content_contains(received, "PINEAPPLE")
        logger.info("Claude SDK tool execution test passed")
