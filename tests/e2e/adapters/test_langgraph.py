"""E2E tests for the LangGraph adapter.

Verifies that the LangGraph adapter can:
- Start, process a message, and stop against a real platform
- Execute platform tools (send_message)

Run with:
    E2E_TESTS_ENABLED=true uv run pytest tests/e2e/adapters/test_langgraph.py -v -s --no-cov
"""
...
class TestLangGraphE2E:
    """E2E tests specific to the LangGraph adapter."""
    ...
    async def test_smoke_responds_to_message(
        ...
        # Subscribe to room for responses before starting agent
        ...
        logger.info(
            "LangGraph smoke test passed: received %d response(s)", len(received)
        )
    ...
    async def test_tool_execution_send_message(
        ...
        logger.info("LangGraph tool execution test passed")
