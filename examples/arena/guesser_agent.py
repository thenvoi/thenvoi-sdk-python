...
Run with (from repo root):
    ...
    uv run examples/arena/guesser_agent.py --config arena_guesser_2 --model gpt-5.2
    uv run examples/arena/guesser_agent.py -c arena_guesser_3 -m claude-opus-4-6
    uv run examples/arena/guesser_agent.py -c arena_guesser_4 -m claude-sonnet-4-6
"""
...
from arena.prompts import create_llm, create_llm_by_name, generate_guesser_prompt
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Guesser agent for 20 Questions")
    parser.add_argument(
        "--config",
        "-c",
