...
Run with (from repo root):
    uv run examples/langgraph/scenarios/04_calculator_as_tool.py

Note: Must be run from repo as it imports standalone/calculator.py
"""
...
from examples.langgraph.standalone.calculator import create_calculator_graph
...
async def main() -> None:
    ...
    calculator_tool = graph_as_tool(
        ...
        # Format successful results and in-band calculator errors consistently.
        result_formatter=lambda state: (
            f"Calculation error: {state['error']}"
            if state.get("error")
            else f"Calculation result: {state['result']}"
        ),
    )
