...
Usage:
    AGENT_CONFIG=/app/config/agent_config.yaml AGENT_KEY=planner python runner.py
"""
...
def load_config(config_path: str, agent_key: str) -> dict[str, Any]:
    ...
    Args:
        ...
        agent_key: Key to look up in keyed config format.
    """
    ...
    # Validate credentials via the SDK config loader (supports keyed and flat formats)
    agent_id, api_key = load_agent_config(agent_key, config_path=path)

    # For keyed format, merge agent-specific fields into the returned config
    agent_section = config.get(agent_key, {})
    if agent_section:
        result = dict(agent_section)
    else:
        result = dict(config)
...
async def main() -> None:
    ...
    # Get config path and agent key from environment
    ...
    # Working directory (env overrides config)
