#!/usr/bin/env python3
"""
Local stdio harness for codex app-server.

Run:
    python scripts/codex_stdio_harness.py
"""
...
from thenvoi.integrations.codex import CodexJsonRpcError, CodexStdioClient, RpcEvent


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Codex stdio harness")
    parser.add_argument("--codex-bin", default="codex", help="Path to codex binary")
    parser.add_argument(
        "--command",
        nargs="+",
        default=None,
        help="Full app-server command (overrides --codex-bin)",
    )
    parser.add_argument(
        "--model",
        default=None,
...
async def handle_server_request(client: CodexStdioClient, event: RpcEvent) -> None:
    if event.id is None:
        return
        ...
        await client.respond(
            event.id,
            {
                "contentItems": [
                    {
                        "type": "inputText",
                        "text": "Harness has no dynamic tool dispatcher configured.",
                    }
                ],
                "success": False,
            },
...
async def run() -> int:
    args = parse_args()
    command = args.command or [args.codex_bin, "app-server", "--listen", "stdio://"]

    client = CodexStdioClient(command=command, cwd=args.cwd)
    final_text = ""
    turn_status = None
    thread_id = None
    chosen_model = None
    ...
    try:
        ...
        init = await client.initialize(
            client_name="thenvoi_codex_stdio_harness",
            client_title="Thenvoi Codex Stdio Harness",
